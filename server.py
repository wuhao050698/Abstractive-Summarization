    ...
    try:
        with open(filepath, 'r',encoding='utf-8') as dict_file:
            for line in dict_file:
                key=line.strip().split(' ')[0] #将原本用空格分开的键和值用冒号分开来，存放在字典中
...
def predict_eval(model, data, params):
    ...
    with codecs.open(params['log_path']+'candidate.txt','w+','utf-8') as f:
        for i in range(len(candidate)):
            f.write(" ".join(candidate[i])+'\n')
            print(" ".join(candidate[i]).replace(' ', '')+'\n')
            ans=" ".join(candidate[i]).replace(' ', '')
    score = {}
    #for metric in config.metrics:
     #   score[metric] = getattr(utils, metric)(reference, candidate, params['log_path'], params['log'], config)
...
def test(input):
    ...
    # print('loading checkpoint...\n')
    checkpoints = torch.load(opt.restore,map_location='cpu')
    dict = load_dict_from_file(config.data+'src.dict')
    temp=''
    for i in input:
        temp+=i
        temp+=' '
    input=temp
    with open(config.data+'valid.src.str', 'w', encoding='utf-8') as f:
        for i in input.split():
            f.write(i)
            f.write(' ')
        f.write('\n')
    with open(config.data+'valid.src.id', 'w', encoding='utf-8') as f:
        for i in input.split():
            if i in dict.keys():
                f.write(dict[i])
                f.write(' ')
        f.write('\n')
    with open(config.data+'valid.tgt.str', 'w', encoding='utf-8') as f:
        for i in input.split():
            f.write(i)
            f.write(' ')
        f.write('\n')
    with open(config.data+'valid.tgt.id', 'w', encoding='utf-8') as f:
        for i in input.split():
            if i in dict.keys():
                f.write(dict[i])
                f.write(' ')
    ...
    if opt.restore:
        params['updates'] = checkpoints['updates']
    score,ans=predict_eval(model, data, params)
    return ans
...
if __name__ == '__main__':
    ...
    for i in list(range(10000)):
        conn,getin= sock.accept()
        tempin=str(conn.recv(1024), encoding = "utf-8")
        if tempin=='':
            continue
        print(tempin)
        ans=test(tempin)
        conn.sendall(bytes(ans, encoding = "utf8"))
        conn.close()
